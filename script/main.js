...
const StatDetection = {
  fromDocument(doc) {
    try {
      ...
      // ADD THIS DEBUG LINE:
      if (SettingsCache.get("traceDice")) {
...
Hooks.once("ready", () => {
  ...
  //diePatched = true;
  //}
  //sfunction removeDiePatch() { if (diePatched && dieOrig) Die.prototype.evaluate = dieOrig; dieOrig = null; diePatched = false; }
  ...
  register(async function wrapper(wrapped, check, context = {}, event2 = null, callback) {
    try {
      ...
      // Optional: Skip if it's initiative - let native PF2e handle it
      // (Your current approach works fine, this is just cleaner)
      // if (kind === "initiative") return await wrapped(check, context, event2, callback);
